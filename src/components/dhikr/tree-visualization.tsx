...
function OliveTree({ cfg, growth, s }: { cfg: TreeStage; growth: number; s: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${s} ${s}`} className='w-full h-full'>
      ...
      <path d={`M${s/2-4} ${s*0.92} Q${s/2-6} ${s*0.92-trunkH*0.5} ${s/2-3} ${canopyY} Q${s/2} ${canopyY-8} ${s/2+3} ${canopyY} Q${s/2+5} ${s*0.92-trunkH*0.5} ${s/2+3} ${s*0.92}`} fill={cfg.trunkColor} stroke={cfg.trunkColor} strokeWidth='1' />
      <circle cx={s/2-canopyR*0.3} cy={canopyY-canopyR*0.2} r={canopyR*0.7} fill={cfg.color} opacity='0.85' />
      <circle cx={s/2+canopyR*0.3} cy={canopyY-canopyR*0.15} r={canopyR*0.65} fill={cfg.color} opacity='0.9' />
      <circle cx={s/2} cy={canopyY-canopyR*0.4} r={canopyR*0.6} fill={cfg.color} opacity='0.95' />
      <circle cx={s/2-canopyR*0.15} cy={canopyY-canopyR*0.35} r={canopyR*0.5} fill={cfg.color} />
      <circle cx={s/2+canopyR*0.15} cy={canopyY-canopyR*0.3} r={canopyR*0.55} fill={cfg.color} opacity='0.9' />
      {hasFruits && [...Array(Math.floor(growth * 8))].map((_, i) => {
        ...
        const cy = (canopyY-canopyR*0.3) + Math.sin(angle) * r * 0.7;
        return <circle key={i} cx={cx} cy={cy} r={s*0.015} fill={cfg.fruitColor} opacity='0.8' />;
      })}
      {hasFlowers && [...Array(4)].map((_, i) => {
        const angle = (i / 4) * Math.PI * 2;
        const r = canopyR * 0.5;
        return <circle key={i} cx={s/2 + Math.cos(angle) * r} cy={(canopyY-canopyR*0.3) + Math.sin(angle) * r * 0.6} r={s*0.012} fill={cfg.flowerColor} />;
      })}
    </svg>
  );
}
...
function PalmTree({ cfg, growth, s }: { cfg: TreeStage; growth: number; s: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${s} ${s}`} className='w-full h-full'>
      ...
      <path d={`M${s/2} ${s*0.92} Q${s/2+8} ${s*0.92-trunkH*0.5} ${s/2+4} ${topY}`} stroke={cfg.trunkColor} strokeWidth={s*0.035} fill='none' strokeLinecap='round' />
      ...
      {[...Array(Math.min(Math.floor(growth * 7) + 1, 7))].map((_, i) => {
        ...
        const cp1y = topY + Math.sin(angle + 0.3) * leafLen * 0.6 - leafLen*0.2;
        const cp2x = s/2 + Math.cos(angle - 0.3) * leafLen * 0.6;
        const cp2y = topY + Math.sin(angle - 0.3) * leafLen * 0.6 - leafLen*0.15;
        return <path key={i} d={`M${s/2} ${topY} Q${cp1x} ${cp1y} ${tipX} ${tipY} Q${cp2x} ${cp2y} ${s/2} ${topY}`} fill={cfg.color} opacity={0.7 + i*0.04} />;
      })}
      {hasFruits && [...Array(3)].map((_, i) => {
        const a = -0.8 + i * 0.4;
        return <circle key={i} cx={s/2 + Math.cos(a) * s*0.03} cy={topY + Math.sin(a) * s*0.03 + s*0.02} r={s*0.018} fill={cfg.fruitColor} />;
      })}
    </svg>
  );
}
...
function GenericTree({ cfg, growth, s }: { cfg: TreeStage; growth: number; s: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${s} ${s}`} className='w-full h-full'>
      ...
      <rect x={s/2-s*0.025} y={canopyY} width={s*0.05} height={trunkH} rx={s*0.01} fill={cfg.trunkColor} />
      {isConifer ? (
        [...Array(Math.min(Math.floor(growth * 4) + 1, 4))].map((_, i) => {
          const w = canopyR * (1.2 - i * 0.15);
          const y = canopyY - i*canopyR*0.35;
          return <polygon key={i} points={`${s/2},${y-canopyR*0.4} ${s/2-w/2},${y+w*0.1} ${s/2+w/2},${y+w*0.1}`} fill={cfg.color} opacity={0.8+i*0.05} />;
        })
      ) : (
        <>
          <circle cx={s/2} cy={canopyY-canopyR*0.3} r={canopyR} fill={cfg.color} opacity='0.8' />
          <circle cx={s/2-canopyR*0.25} cy={canopyY-canopyR*0.15} r={canopyR*0.75} fill={cfg.color} opacity='0.9' />
          <circle cx={s/2+canopyR*0.25} cy={canopyY-canopyR*0.2} r={canopyR*0.7} fill={cfg.color} />
          {growth >= 0.4 && <circle cx={s/2} cy={canopyY-canopyR*0.5} r={canopyR*0.5} fill={cfg.color} opacity='0.95' />}
        </>
      )}
      {growth >= 0.6 && !isConifer && [...Array(Math.floor(growth * 6))].map((_, i) => {
        const angle = (i / 6) * Math.PI * 2;
        const r = canopyR * 0.6;
        return <circle key={i} cx={s/2 + Math.cos(angle) * r} cy={(canopyY-canopyR*0.3) + Math.sin(angle) * r * 0.7} r={s*0.012} fill={i % 2 === 0 ? cfg.flowerColor : cfg.fruitColor} opacity='0.9' />;
      })}
    </svg>
  );
}

/* ========== Orbiting Lights Component ========== */
function OrbitingLights({ size, color, growth, count = 5 }: { size: number; color: string; growth: number; count?: number }) {
  if (growth < 0.2) return null;
  const center = size / 2;
  ...
  return (
    <>
      {[...Array(actualCount)].map((_, i) => {
        const delay = i * (8 / actualCount);
        const duration = 6 + i * 1.5;
        const orbitRadius = radius * (0.85 + (i % 3) * 0.08);
        const particleSize = 2 + (i % 2) * 1.5;
        const hue = i % 2 === 0 ? color : '#fbbf24';
        return (
          <motion.div
            key={i}
            ...
            style={{
              ...
              height: particleSize,
              background: hue,
              boxShadow: `0 0 ${particleSize * 3}px ${hue}, 0 0 ${particleSize * 6}px ${hue}40`,
            }}
            animate={{
              x: [
                Math.cos((delay / duration) * Math.PI * 2) * orbitRadius,
                Math.cos(((delay + duration * 0.25) / duration) * Math.PI * 2) * orbitRadius,
                Math.cos(((delay + duration * 0.5) / duration) * Math.PI * 2) * orbitRadius,
                Math.cos(((delay + duration * 0.75) / duration) * Math.PI * 2) * orbitRadius,
                Math.cos(((delay + duration) / duration) * Math.PI * 2) * orbitRadius,
              ],
              y: [
                Math.sin((delay / duration) * Math.PI * 2) * orbitRadius * 0.6,
                Math.sin(((delay + duration * 0.25) / duration) * Math.PI * 2) * orbitRadius * 0.6,
                Math.sin(((delay + duration * 0.5) / duration) * Math.PI * 2) * orbitRadius * 0.6,
                Math.sin(((delay + duration * 0.75) / duration) * Math.PI * 2) * orbitRadius * 0.6,
                Math.sin(((delay + duration) / duration) * Math.PI * 2) * orbitRadius * 0.6,
              ],
...
export default function TreeVisualization({ treeType, level, size = 200 }: TreeVizProps) {
  ...
  const swayAmount = growth > 0.1 ? 2 : 0;
  const swayDuration = 3 + Math.random();
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          inset: -size * 0.15,
          background: `radial-gradient(circle, ${cfg.color}20, ${cfg.color}08 50%, transparent 70%)`,
        }}
        animate={growth > 0.3 ? {
          scale: [1, 1.15, 1],
          opacity: [0.5, 0.8, 0.5],
      ...
      {/* Orbiting lights around the tree */}
      <OrbitingLights size={s} color={cfg.color} growth={growth} count={level >= 7 ? 8 : level >= 4 ? 6 : 4} />
    </motion.div>
  );
}
